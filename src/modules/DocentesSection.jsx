...
function DocentesSection({ ciclo, onCicloChange, docentesData,
  isActive,
  sectionRef,
  startingChartRef,
  currentChartRef,
  ...
  const {
    ...
    setControl,
    filters,
  } = useFilters({ ciclo: docentesData.meta.ciclo })

  const nivelesFiltrados = useMemo(
    () => filtrarNivelesDocentes(docentesData, filters),
    [filters, docentesData],
  )
  ...
  const getControlDisplayValue = (totales, key) =>
    control === 'todos'
      ? totales?.[key] ?? 0
      : key === control
      ? totales?.[key] ?? 0
    ...
    >
      ...
      <div style={{ marginTop: '50px' }}>
        <h3 className="chart-title" style={{ fontSize: '1.8em' }}>
          Detalle por Nivel y Vertiente
        </h3>
        <div className="stats-table-container">
          <table
            className="stats-table"
            aria-label="Tabla detallada de docentes por nivel y vertiente"
          >
            <thead>
              <tr>
            ...
            <tbody>
              ...
              {showingGlobalTotals && (
                <>
                  <tr>
                    ...
                    <td>
                      {formatNumber(docentesData.modalidades.escolarizada.estatal)}
                    </td>
                    <td>
                      {formatNumber(
                        docentesData.modalidades.escolarizada.federalizado,
                      )}
                    </td>
                    <td>
                      {formatNumber(docentesData.modalidades.escolarizada.federal)}
                    </td>
                    <td>
                      {formatNumber(docentesData.modalidades.escolarizada.autonomo)}
                    </td>
                    <td>
                      {formatNumber(docentesData.modalidades.escolarizada.total)}
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td>
                      {formatNumber(
                        docentesData.modalidades.no_escolarizada.estatal,
                      )}
                    </td>
                    <td>
                      {formatNumber(
                        docentesData.modalidades.no_escolarizada.federalizado,
                      )}
                    </td>
                    <td>
                      {formatNumber(
                        docentesData.modalidades.no_escolarizada.federal,
                      )}
                    </td>
                    <td>
                      {formatNumber(
                        docentesData.modalidades.no_escolarizada.autonomo,
                      )}
                    </td>
                    <td>
                      {formatNumber(
                        docentesData.modalidades.no_escolarizada.total,
                      )}
                    </td>
                  </tr>
                </>
              ...
              <tr className="total-row">
                ...
                {CONTROL_KEYS.map((key) => (
                  <td key={key}>
                    {formatNumber(showingGlobalTotals ? docentesData.controles_totales[key] : macroSummaries.overall[key])}
                  </td>
                ))}
                <td>{formatNumber(showingGlobalTotals ? docentesData.meta.total_general : macroSummaries.overall.total)}</td>
              </tr>
            </tbody>
          </table>
        </div>
